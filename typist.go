...
var contStr = "Press Enter to continue!"
var tryAgainStr = "Press Enter to try again!"
var welcomeStr = "Hi! Welcome to Funny Typist! \nLet's having Fun!\n"
...
func choseLesson(size int) int {
	clear()
	green.Printf("Which lesson do you want to try?\nPlease input the number(%d~%d):", 1, len(strs))
	num := 0
	for {
...
func showResult(correct int, size int) bool {
	pass := true
	rate := int(float64(correct) / float64(size) * 100)
	if rate >= 97 {
		cyan.Print("\n\nGreat!")
	} else if rate >= 90 {
		cyan.Print("\n\nGood!")
	...
	cyan.Printf(" Your correct rate is %d%%\n\n", rate)
	if rate > 97 {
		cyan.Print(contStr)
	} else {
		cyan.Print(tryAgainStr)
	}
	for {
...
func readIdx() int {
	body, err := ioutil.ReadFile(idxFile)
	if err == nil {
		idx, _ := strconv.Atoi(strings.TrimSpace(string(body)))
...
func writeIdx(idx int) {
	err := ioutil.WriteFile(idxFile, []byte(fmt.Sprintf("%d", idx)), 0777)
	if err != nil {
		panic(err)
	}
}
...
func main() {
	...
	clear()
	for idx := oIdx; idx < len(strs); idx++ {
		writeIdx(idx)
		str := strs[idx]
		green.Printf("Lesson %d\n\n", idx+1)
		cyan.Println(str)
		bs := []rune(str)
		correct := 0
		rechose := false
